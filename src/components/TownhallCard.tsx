...
const TownhallCard: React.FC<TownhallCardProps> = ({
  image,
  title,
  description,
}) => {
  ...
  const handleHoverStart = () => {
    ...
    titleControls.start({ translateY: -110 });
  ...
  const handleHoverEnd = () => {
    ...
    titleControls.start({ translateY: 0, fontSize: "26px" });
  ...
  return (
    ...
    >
      <div className="items-center gap-3 inline-flex flex-col justify-center relative">
        <motion.div initial={{ scale: 1, opacity: 1 }} animate={imageControls}>
          <Image
        ...
        <div className="flex-col items-center inline-flex justify-between relative">
          <motion.div
            className="relative w-fit mx-2 [font-family:'Outfit-SemiBold',Helvetica] font-semibold text-center leading-8 "
            initial={{ translateY: 0, fontSize: "26px" }}

		...
		<div className='size-full h-screen absolute top-0  left-0 bg-[#191E23] '>
			...
			<div className=' mx-0 flex flex-col justify-between items-start  border-t mt-14 gap-12'>
				<nav className='flex flex-col w-full '>
					<motion.div
						...
						<Link
							...
							legacyBehavior
							passHref>
							<h1 className={` ${pathname === '/' ? 'text-primary' : 'text-white'}`}>HOME</h1>
						</Link>
					</motion.div>
					<motion.div
						className='border-b p-4 w-full cursor-pointer'
						...
						<Accordion
							...
							<AccordionItem value='item-1'>
								<AccordionTrigger className={pathname === '/products/bountybird' || pathname === '/products/townhall' ? 'text-primary' : 'text-white'}>PRODUCTS</AccordionTrigger>
								<AccordionContent>
												...
												<Link
													...
													<div className='flex-col items-start justify-center gap-[3px] flex-[0_0_auto] inline-flex relative'>
														<div className='items-center gap-[6px] flex-[0_0_auto] inline-flex relative'>
															<div className=' font-semibold text-white text-sm whitespace-nowrap'>{item.title}</div>
															{item.coming_soon ? (
																<span className='rounded-full text-xs bg-destructive px-1.5'>Coming Soon</span>
															) : (
																<Image
					...
					<motion.div
						...
						<Link
							...
							legacyBehavior
							passHref>
							<h1 className={` ${pathname === '/infrastructure' ? 'text-primary' : 'text-white'}`}>INFRASTRUCTURE</h1>
						</Link>
					</motion.div>
					<motion.div
						className='border-b p-4 w-full cursor-pointer'
						...
						<Link
							...
							legacyBehavior
							passHref>
							<h1 className={` ${pathname === '/contact' ? 'text-primary' : 'text-white'}`}>CONTACT</h1>
						</Link>
					</motion.div>
				</nav>
				<div className='flex flex-col justify-center w-full items-center gap-4'>
					<motion.div
						variants={itemVariants}
						className=' transition-all p-4 w-full  rounded-full'>
